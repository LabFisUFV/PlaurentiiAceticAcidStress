...
adapterLocation = fullfile(findGECKOroot,'ecpapla_GEM','ecpapla_GEMAdapter.m');
...
saveEcModel(ecModel,'ecpaplaGEM_stage1.yml');

%% STAGE 2: integration of kcat into the ecModel structure
%ecModel=loadEcModel('ecpaplaGEM_stage1.yml'); % Uncomment if you want to
%reload model.
...
ecModel_DLKcat = selectKcatValue(ecModel, kcatList_DLKcat);
...
ecModel_DLKcat = getKcatAcrossIsozymes(ecModel_DLKcat);
...
[ecModel_DLKcat, rxnsMissingGPR_DLKcat, standardMW_DLKcat, standardKcat_DLKcat] = getStandardKcat(ecModel_DLKcat);
...
ecModel_DLKcat = applyKcatConstraints(ecModel_DLKcat);
...
ecModel_DLKcat = setProtPoolSize(ecModel_DLKcat,Ptot,f,sigma);
...
saveEcModel(ecModel_BRENDA,'ecpaplaGEM_stage2_BRENDA_wostd.yml');
saveEcModel(ecModel_DLKcat,'ecpaplaGEM_stage2_DLKcat_wostd.yml');
saveEcModel(ecModel_merged,'ecpaplaGEM_stage2_merged_wostd.yml');

%% STAGE 3: model tuning
%ecModel=loadEcModel('ecpaplaGEM_stage2_merged_wostd.yml'); % Uncomment if you want to
%reload model.
...
% The growth rate is far below 0.42 (the maximum growth rate of
% P. laurentii, that is entered in the model adapter as obj.params.gR_exp.
...
%rxnIdx = find(strcmp(kcatList_merged.rxns,'r_0450'));
...
saveEcModel(ecModel,'ecpaplaGEM_stage3_DLKcat_wostd.yml');

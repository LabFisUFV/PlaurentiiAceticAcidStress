...
model   = setParam(model,'lb','r_2111',0.045);
%model   = setParam(model,'obj','r_2111',1);
model   = setParam(model,'obj','r_2111',0);
%model   = setParam(model,'obj','r_4046',1);


model = setParam(model, 'lb', {'r_1718'}, (-0.678-0.010)); % open xylose
model = setParam(model, 'ub', {'r_1718'}, (-0.678+0.010)); % open xylose
...
model = setParam(model, 'var', {'r_1992'}, -1.6266, 10);    % O2
model = setParam(model, 'var', {'r_1672'}, 1.8585, 10);    % CO2

model.lb(strcmp(model.rxns, 'prot_pool_exchange')) = -(Ptot*f*sigma*1000);
model = setParam(model, 'eq', {'r_1714'}, 0); % close glucose
model = setParam(model, 'lb', {'r_4046'}, 0.05) %change lower bound NGAM

model = setParam(model, 'var', 'prot_pool_exchange', -99.175, 10);

model   = setParam(model,'var','r_2111',0.045,5);
%model   = setParam(model,'obj','r_2111',1);
%model   = setParam(model,'obj','r_2111',0);
...
writetable(struct2table(out),'out_ATS_wo.csv');
...
writetable(struct2table(outMapped),'out_Mapped_ATS_wo.csv');
...
writecell(outATP_median,'out_Mapped_ATS_wo_ATP_median.csv');
...
writecell(outATP_mean,'out_Mapped_ATS_wo_ATP_mean.csv');
...
writecell(outNADH_median,'out_Mapped_ATS_wo_NADH_median.csv');
...
writecell(outNADH_mean,'out_Mapped_ATS_wo_NADH_mean.csv');
...
writecell(outNADPH_median,'out_Mapped_ATS_wo_NADPH_median.csv');
...
writecell(outNADPH_mean,'out_Mapped_ATS_wo_NADPH_mean.csv');

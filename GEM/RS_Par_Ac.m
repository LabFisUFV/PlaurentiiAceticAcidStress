...
model   = setParam(model,'lb','r_2111',0.056);
%model   = setParam(model,'obj','r_2111',1);
model   = setParam(model,'obj','r_2111',0);
%model   = setParam(model,'obj','r_4046',1);


model = setParam(model, 'lb', {'r_1718'}, (-0.554-0.009)); % open xylose
model = setParam(model, 'ub', {'r_1718'}, (-0.554+0.009)); % open xylose

model = setParam(model, 'lb', {'r_1634'}, (-0.757-0.007)); % open acetate
model = setParam(model, 'ub', {'r_1634'}, (-0.757+0.007)); % open acetate
...
model = setParam(model, 'var', {'r_1992'}, -2.2179, 10);    % O2
model = setParam(model, 'var', {'r_1672'}, 2.4992, 10);    % CO2

model.lb(strcmp(model.rxns, 'prot_pool_exchange')) = -(Ptot*f*sigma*1000);
model = setParam(model, 'eq', {'r_1714'}, 0); % close glucose
model = setParam(model, 'lb', {'r_4046'}, 0.05); %change lower bound NGAM

model = setParam(model, 'var', 'prot_pool_exchange', -90.2, 10);
...
writetable(struct2table(out),'out_Par_Ac.csv');
...
writetable(struct2table(outMapped),'out_Mapped_Par_Ac.csv');
...
writecell(outATP_median,'out_Mapped_Par_Ac_ATP_median.csv');
...
writecell(outATP_mean,'out_Mapped_Par_Ac_ATP_mean.csv');
...
writecell(outNADH_median,'out_Mapped_Par_Ac_NADH_median.csv');
...
writecell(outNADH_mean,'out_Mapped_Par_Ac_NADH_mean.csv');
...
writecell(outNADPH_median,'out_Mapped_Par_Ac_NADPH_median.csv');
...
writecell(outNADPH_mean,'out_Mapped_Par_Ac_NADPH_mean.csv');

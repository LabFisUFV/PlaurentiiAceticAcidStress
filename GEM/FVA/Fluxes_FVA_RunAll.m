...
model = changeRxnBounds(model, 'EX_glc__D_e', -10, 'l'); %D-glucose
...
toRemove1 = find(contains(model.rxns,'prot_'));
toRemove1 = model.rxnNames(toRemove1);
toRemove2 = find(contains(model.rxns,'subpool_'));
...
varNamesT = {'enzymes' 'ranges' 'minUsage' 'maxUsage'};
FVAtable  = table(rxnNamesList,ranges,minFlux,maxFlux,'VariableNames', varNamesT);
FVA_filename = "FVA_fluxes_eciML1515u_CORAL_DLKcat_noBio_Und.csv";
writetable(FVAtable, FVA_filename, 'Delimiter','\t')
fprintf('\n');
fprintf('Export finished');
fprintf('\n');
...
varNamesT = {'enzymes' 'ranges' 'minUsage' 'maxUsage'};
FVAtable  = table(rxnNamesList,ranges,minFlux,maxFlux,'VariableNames', varNamesT);
FVA_filename = "FVA_fluxes_eciML1515u_CORAL_DLKcat_Bio_noUnd.csv";
writetable(FVAtable, FVA_filename, 'Delimiter','\t')
fprintf('\n');
fprintf('Export finished');
fprintf('\n');
...
varNamesT = {'enzymes' 'ranges' 'minUsage' 'maxUsage'};
FVAtable  = table(rxnNamesList,ranges,minFlux,maxFlux,'VariableNames', varNamesT);
FVA_filename = "FVA_fluxes_eciML1515u_CORAL_DLKcat_Bio_Und.csv";
writetable(FVAtable, FVA_filename, 'Delimiter','\t')
fprintf('\n');
fprintf('Export finished');
fprintf('\n');

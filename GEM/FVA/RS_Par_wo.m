...
%Biomass_specific models with data from Almeida et al. (2022)
%First, it was necessary to adjust the dilution rate (ub to 0.055) and NGAM (lb to 0.5) to allow
...
%Using the model adjusted by Samuel (elemental and charge balance)

%For some reason, all reactions are non-essential in the model (all
%versions)
...
model   = setParam(model,'lb','r_2111',0.043);
